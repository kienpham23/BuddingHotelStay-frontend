<template>
  <div class="auth-page">
    <div class="auth-box">
      ...
      <!-- LOGIN SECTION -->
      <div v-if="!showForgot" style="width:100%;">
        ...
        <form @submit.prevent="handleLogin" class="auth-form">
          ...
          <div class="field-group">
            ...
            <div class="input-wrap">
              ...
              <button type="button" class="eye-btn" @click="showPwd = !showPwd">
                <span v-if="!showPwd">{{ $t('auth.show_pwd') }}</span>
                <span v-else>{{ $t('auth.hide_pwd') }}</span>
              </button>
            </div>
          </div>

          <div class="remember-row" style="display: flex; justify-content: flex-end; margin-top: -4px; margin-bottom: 8px;">
            <a href="#" class="forgot-link" style="color: #3b82f6; font-size: 13px; font-weight: 600; text-decoration: none;" @click.prevent="showForgot = true">{{ $t('auth.forgot_password_link') }}</a>
          </div>

          <div v-if="error" class="error-msg">{{ error }}</div>

          <button type="submit" class="btn-primary" :disabled="loading">
            <span v-if="!loading">{{ $t('auth.login_btn') }}</span>
          ...
          <p class="switch-text">
            {{ $t('auth.no_account') }}
            <RouterLink to="/register">{{ $t('auth.create_account') }}</RouterLink>
          </p>
        </form>
      </div>
      ...
      <div v-else style="width:100%;">
        ...
        <form @submit.prevent="handleForgotPassword" class="auth-form">
          ...
          <button type="submit" class="btn-primary" :disabled="loading || (forgotStep === 2 && forgotPwdMismatch)">
            <span v-if="!loading">
          ...
          <p class="switch-text">
            <a href="#" @click.prevent="showForgot = false">{{ $t('auth.back_to_login') }}</a>
          </p>
        </form>
      </div>
    </div>
...
// Forgot Password states & methods
...
const handleLogin = async () => {
  ...
  try {
    ...
    else if (role === 'CUSTOMER') router.push('/bookings')
    else router.push('/')
  } catch (e) {
    const apiMsg = e.response?.data?.message
    if (apiMsg === 'Bad credentials') {
      error.value = t('auth.errors.bad_credentials')
    } else {
      error.value = apiMsg || t('auth.errors.login_failed_unknown')
    }
  } finally {
    loading.value = false
  }
